...
RUN apt-get update -q -q && \
  ...
  apt-get update -q -q && \
  apt-get --no-install-recommends --yes --force-yes install postgresql-10 postgresql-10-postgis-3 postgresql-10-postgis-3-scripts && \
...
COPY ./postgresql9.3 /etc/postgresql/10
