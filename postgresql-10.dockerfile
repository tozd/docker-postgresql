...
RUN apt-get update -q -q && \
 apt-get --yes install wget ca-certificates gnupg && \
 ...
 apt-get --no-install-recommends --yes install postgresql-10 postgresql-10-postgis-2.4 postgresql-10-postgis-2.4-scripts && \
 mkdir -m 700 /var/lib/postgresql.orig && \
 mv /var/lib/postgresql/* /var/lib/postgresql.orig/ && \
 echo 'mappostgres postgres postgres' >> /etc/postgresql/10/main/pg_ident.conf && \
 echo 'mappostgres root postgres' >> /etc/postgresql/10/main/pg_ident.conf && \
...
COPY ./etc /etc
COPY ./postgresql /etc/postgresql/10
