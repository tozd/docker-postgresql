...
RUN apt-get update -q -q && \
 ...
 echo 'mappostgres gis gis' >> /etc/postgresql/9.3/main/pg_ident.conf && \
 echo 'mappostgres root gis' >> /etc/postgresql/9.3/main/pg_ident.conf && \
 echo 'hostssl all all 0.0.0.0/0 md5' >> /etc/postgresql/9.3/main/pg_hba.conf && \
 echo 'local all postgres peer map=mappostgres' >> /etc/postgresql/9.3/main/pg_hba.conf
