...
RUN apt-get update -q -q && \
  ...
  apt-get update -q -q && \
  apt-get --no-install-recommends --yes --force-yes install postgresql-18 postgresql-18-postgis-3 postgresql-18-postgis-3-scripts && \
  mkdir -m 700 /var/lib/postgresql.orig && \
  mv /var/lib/postgresql/* /var/lib/postgresql.orig/ && \
  echo 'mappostgres postgres postgres' >> /etc/postgresql/18/main/pg_ident.conf && \
  echo 'mappostgres root postgres' >> /etc/postgresql/18/main/pg_ident.conf && \
  ...
  sed -r -i 's/local\s+all\s+postgres\s+peer/local all postgres peer map=mappostgres/' /etc/postgresql/18/main/pg_hba.conf && \
  echo "include_dir = 'conf.d'" >> /etc/postgresql/18/main/postgresql.conf && \
  mkdir -p /var/run/postgresql/18-main.pg_stat_tmp && \
  chown postgres:postgres /var/run/postgresql/18-main.pg_stat_tmp && \
  apt-get clean && rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/* ~/.cache ~/.npm

COPY ./etc/service/postgresql /etc/service/postgresql
COPY ./log /etc/service/postgresql/log
COPY ./postgresql /etc/postgresql/18
