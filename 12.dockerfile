...
RUN apt-get update -q -q && \
  ...
  apt-get update -q -q && \
  apt-get --no-install-recommends --yes --force-yes install postgresql-12 postgresql-12-postgis-3 postgresql-12-postgis-3-scripts && \
  mkdir -m 700 /var/lib/postgresql.orig && \
  mv /var/lib/postgresql/* /var/lib/postgresql.orig/ && \
  echo 'mappostgres postgres postgres' >> /etc/postgresql/12/main/pg_ident.conf && \
  echo 'mappostgres root postgres' >> /etc/postgresql/12/main/pg_ident.conf && \
  echo 'host all all 0.0.0.0/0 md5' >> /etc/postgresql/12/main/pg_hba.conf && \
  echo 'hostssl all all 0.0.0.0/0 md5' >> /etc/postgresql/12/main/pg_hba.conf && \
  sed -r -i 's/local\s+all\s+postgres\s+peer/local all postgres peer map=mappostgres/' /etc/postgresql/12/main/pg_hba.conf && \
  echo "include_dir = 'conf.d'" >> /etc/postgresql/12/main/postgresql.conf && \
  mkdir -p /var/run/postgresql/12-main.pg_stat_tmp && \
  chown postgres:postgres /var/run/postgresql/12-main.pg_stat_tmp && \
  apt-get clean && rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/* ~/.cache ~/.npm

COPY ./etc/service/postgresql /etc/service/postgresql
COPY ./log-9.3 /etc/service/postgresql/log
COPY ./postgresql-9.3 /etc/postgresql/12
